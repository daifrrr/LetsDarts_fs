...
let parseThrow (throw: string) : Shot option =
        let r =
            System.Text.RegularExpressions.Regex(@"^(?<character>[dst]{1})(?<number>\d{1,2})$",
                                                 System.Text.RegularExpressions.RegexOptions.IgnoreCase)

        let m = r.Match(throw.ToLower())

        match (m.Success, System.Char.TryParse m.Groups["character"].Value,
               System.Int32.TryParse m.Groups["number"].Value) with
